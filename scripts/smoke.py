...
t=r.json()['token'];h={'Authorization':'Bearer '+t};k=('railway-'+uuid.uuid4().hex).encode();v=b'immudb persistence probe';w=requests.post(b+'/api/v2/db/set',headers=h,json={'KVs':[{'key':enc(k),'value':enc(v)}]},timeout=30);assert w.status_code==200,w.text
